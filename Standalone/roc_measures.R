############################################################################
# ROC MEASURES
# Calculate and store ROC measures for a multiclass classification problem
# Class ROC stores ROC measures for a single class
# Class ROCMultiClass stores ROC measures for CIR, CON, LN, LX
############################################################################
...
ROC = R6Class("ROC", list(
		roc_vals = list(),
		out_dir = "/srv/scratch/z8039617",
...
ROCMultiClass = R6Class("ROCMultiClass", list(
	...
	write = function(result_file, suffix = NULL) {
		if (!is.null(suffix)) {
	...
	plot = function(method, title, out_file, result_file = NA) {
#		print("Plotting")
		...
		out_file = paste0(out_file, "_", method)
#		if (ncol(data[, grepl("acc", colnames(data)), drop = FALSE]) > 2) {
		if (ncol(data) > 4) {
	...
	getROCVals = function(method, pos_class, neg_class, tr) {
		print(paste0("Pos class = ", pos_class, ", Neg class = ", neg_class))
#		print("Truth")
#		print(tr$truth)
#		print("Response")
#		print(tr$response)
		...
		if (length(diff) > 0) {
			print("Truth and response have different levels!")
			print(tr$truth)
			print(tr$response)
			...
			print(paste0("tp = ", tp, ", tn = ", tn, ", fp = ", fp, ", fn = ", fn, ", sens = ", sensr, ", spec = ", specr, ", f1 = ", f1, ", accuracy = ", acc))
		}
	...
	calculate = function(method, predn, class_names = NULL) {
		print("Calculating ROC measures")
...
#			neg_class = "REST"		# When do we need to do this? FIX ME
	...
	calc = function(truth, response, class_names) {
		...
		print(diff)
		if (length(diff) > 0) {
			print("Truth and response have different levels in calc")
			print(truth)
			print(response)
		...
		if (length(class_names) == 2) {
			...
			print(paste0("Negative class: ", neg_class))
			tr = list("truth" = truth, "response" = response)
			roc_measures[[1]] = self$getROCVals("combn", pos_class, neg_class, tr)
#			pos_class = ifelse(levels(class_names)[1] == neg_class, levels(class_names)[2], levels(class_names)[1])
		} else {			
			for (j in 1:length(class_names)) {
				print(paste0("j = ", j, ", Class : ", class_names[[j]]))
	...
	calc_mc_roc = function(truth, response) {
		...
		resp = data.frame(l)
		auroc = multiclass.roc(response = truth, predictor = resp)
		print(auroc$auc)
