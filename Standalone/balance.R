...
#				if (class_sizes[cls] < max_class_size / 1.5) {
				if (class_sizes[cls] < max_class_size) {
					...
					if (bal_method == "SMOTE") {
							bal_task = smote(binary_task, rate = rate, nn = nn)
			...
			# Not sure if this is needed now, but put it back and see if it makes a difference
...
#			print(rownames(balanced_dat))
  ...
  makePreprocWrapper(
    ...
    par.set = makeParamSet(
			makeCharacterParam("target_name"),
			makeDiscreteParam("bal_method", values = c("SMOTE", "OVERSAMPLE"))
    ),
