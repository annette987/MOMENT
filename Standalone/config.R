############################################################################
# CONFIG
# Read and store config parameters for an ML task
############################################################################
...
new_config_data = function(base_data) {
	structure(
		list(
			...
			inclusions   = base_data['Inclusions'],
...
new_config_single = function(config_data, row) {
	structure(
		list(
			...
			inclusions   = config_data[row, 'Inclusions'],
			activeLrn    = eval(parse(text = config_data[row, 'ActiveLearners']))
		),
		class = "config_single"
	)
...
config = function(filename) {
	...
	if (file_ext(filename) == "csv") {
		...
		config_data = as.data.frame(read_excel(filename, sheet = "Main"))
		print(config_data)
		cf = new_config(config_data, 1)	
		base_data = as.data.frame(read_excel(filename, sheet = "BaseModels"))
		print(base_data)
		cf$baseModels = apply(base_data, 1, new_config_base)
		
		if ("Datasets" %in% sheet_names) {
			data_sets = as.data.frame(read_excel(filename, sheet = "Datasets"))
		...
		if ("ValidationSets" %in% sheet_names) {
			validation_sets = as.data.frame(read_excel(filename, sheet = "ValidationSets"))
			cf$validationSets = apply(validation_sets, 1, new_config_data)
		}
		
		if ("Experts" %in% sheet_names) {
			experts = as.data.frame(read_excel(filename, sheet = "Experts"))
