#' @export
makeRLearner.classif.ggbm = function() {
  makeRLearnerClassif(
    cl = "classif.ggbm",
    package = "gbm",
...
#' @export
trainLearner.classif.ggbm = function(.learner, .task, .subset, .weights = NULL, ...) {
	print("Training ggbm")
  td = getTaskDesc(.task)
...
#' @export
predictLearner.classif.ggbm = function(.learner, .model, .newdata, ...) {
	print("Predicting ggbm")
#	print(.newdata)
#	print("Arguments:")
#	print(list(...))
	print(paste0("Number of rows in newdata: ", nrow(.newdata)))
  td = .model$task.desc
  m = .model$learner.model
#	print("n.trees")
#	print(m$n.trees)
  p = gbm::predict.gbm(m, newdata = .newdata, type = "response", n.trees = m$n.trees, single.tree = FALSE, ...)
	print(paste0("Number of rows in p: ", nrow(p)))
	print(class(p))
	print(str(p))
	print("P is")
	print(p)
