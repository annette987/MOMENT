...
#	print(paste0("Normalising data: ",  method))
	if (ncol(data) == 0) {
		final = data
	} else {
...
#		print(numeric_cols)
		...
		if (length(numeric_cols) == 0) {
			...
			if (method == "MINMAX") {
					...
					norm_dat = as.data.frame(normalize.quantiles(data.matrix(data[, numeric_cols, drop = FALSE])))
...
#			print(head(final))
#			print("Non-numeric:")
#			print(data[!numeric_cols, drop = FALSE])
...
#			print("Normalising test data")
  ...
  makePreprocWrapper(
    ...
    par.set = makeParamSet(
			makeDiscreteParam("method", values = c("NONE", "STAND", "LOGT", "MINMAX", "CPM", "CPM_LOGT", "QUANTILE", "ZSCORE"))
    ),
