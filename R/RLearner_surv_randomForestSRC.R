...
makeRLearner.surv.randomForestSRC = function() {
  makeRLearnerSurv(
    ...
    par.set = ParamHelpers::makeParamSet(
      ...
      ParamHelpers::makeDiscreteLearnerParam(id = "var.used", default = FALSE, tunable = FALSE,
...
trainLearner.surv.randomForestSRC = function(.learner, .task, .subset, .weights = NULL, ...) {
  ...
  randomForestSRC::rfsrc(f, data = mlr::getTaskData(.task, subset = .subset), case.wt = .weights, ...)
}
